...
VICTIM INFORMATION SAFEGUARDS (added 2026-08-07) -- CourtBridge tracks program participants only, never victims. Every notes field and document upload shows a warning about this before you type or upload. Document uploads require you to confirm the file was reviewed and contains no victim-identifying information before the upload button will work. If victim info gets entered anywhere by mistake, click the "Flag as possible victim information" button on that record -- it is immediately hidden from everyone except an org admin/founder (not deleted). Org admins and founders review flagged records on the new Victim Info Review screen in the sidebar.

COURT REPORTING (new screen, added 2026-08-07) -- captures the statutory data fields Florida's 2026 problem-solving court reporting law will eventually require (program entry, ongoing compliance, program exit), with a full audit trail separating what actually happened from your compliance decision about it. Generates a participant compliance summary and a CSV export -- both intentionally limited to enrollment, attendance, requirements completed, compliance status, and discharge status only, since that is what a court legitimately needs. Not an official OSCA submission -- the state has not published its final reporting format yet (targeted March 2028).

MAP VIEW / LOCATION DATA -- most screens now show "GPS Verified" instead of raw coordinates. Exact coordinates only appear on Map View itself and through explicit "view on map" links, and both are logged (who viewed, when) for audit purposes.

RECORDS DISCLOSURE REVIEW (added 2026-08-07) -- approving a court/PO/agency records request now requires an org admin or founder to answer a required review (participant consent, legal authority, whether 42 CFR Part 2 applies, minimum necessary information) and personally attest to the decision before the Approve button will work. Ordinary staff can view and flag requests but can no longer approve or deny them.
