...
import ClientInvites from './screens/ClientInvite'
import Settings from './screens/Settings'
import Affirmations from './screens/Affirmations'
import Documents from './screens/DocumentUpload'
import Messages from './screens/Messages'
import ContactLog from './screens/ContactLog'
import ClientCheckins from './screens/CheckInHistory'
import MapView from './screens/MapView'
import Reports from './screens/Reports'
import Violations from './screens/ViolationReport'
...
export default function App() {
  ...
  const [expandedMenus, setExpandedMenus] = useState({
    clientCompliance: false, clients: false, operations: false, admin: false
  })
  ...
  function renderMain() {
    switch (currentSection) {
      ...
      case 'enrollclient': return <ClientInvites session={session} />
      ...
      case 'sops': return <SOP session={session} />
      case 'providerguide': return <ProviderGuide session={session} />
      case 'orgsettings': return <OrgAdmin session={session} />
  ...
  const NavItem = ({ id, label, sub = false }) => (
    <div onClick={() => navTo(sub ? activeSection : id, sub ? id : null)}
      style={{ padding: sub ? '8px 16px 8px 32px' : '10px 16px', fontSize: sub ? 13 : 14, fontWeight: currentSection === id ? 600 : 400, color: currentSection === id ? '#fff' : sub ? '#A8C4E0' : '#C8D8EE', background: currentSection === id ? 'rgba(255,255,255,0.15)' : 'transparent', borderRadius: 6, cursor: 'pointer', marginBottom: 2, borderLeft: currentSection === id ? '3px solid #fff' : '3px solid transparent', transition: 'all 0.15s' }}>
      {label}
    </div>
  )
    ...
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: 'Arial, sans-serif', background: '#F5F6F8' }}>
      ...
      <div style={{ width: 220, minHeight: '100vh', background: BLUE, position: 'fixed', top: 0, left: sidebarOpen ? 0 : -220, zIndex: 50, transition: 'left 0.25s', overflowY: 'auto', display: 'flex', flexDirection: 'column' }} className="sidebar">
        ...
        <div style={{ flex: 1, padding: '12px 8px' }}>
          ...
          {expandedMenus.clients && (<>
            ...
            <NavItem id="enrollclient" label="Enroll New Client" sub />
      ...
      <div style={{ flex: 1, marginLeft: 0, display: 'flex', flexDirection: 'column' }}>
        <div style={{ background: '#fff', padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 16, borderBottom: '1px solid #eee', position: 'sticky', top: 0, zIndex: 30 }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 600, color: BLUE }}>{currentSection.charAt(0).toUpperCase() + currentSection.slice(1).replace(/([A-Z])/g, ' $1')}</div>
        </div>
        <div style={{ flex: 1, padding: 20 }}>{renderMain()}</div>
      </div>
      <style>{`@media (min-width: 768px) { .sidebar { left: 0 !important; } .sidebar + div { margin-left: 220px !important; } }`}</style>
    </div>
  )
}
